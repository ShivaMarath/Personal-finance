...
import { Transaction, RecurringInterval, User, Account, Budget, Prisma } from "@/lib/generated/prisma";
...
// Type for transaction with account
type TransactionWithAccount = Transaction & {
  account: Account;
};

// Type for monthly stats
type MonthlyStats = {
  totalExpenses: number;
  totalIncome: number;
...
export const processRecurringTransaction = inngest.createFunction(
  ...
  async ({ event, step }: { event: RecurringTransactionEvent; step: any }) => {
    ...
    if (!event?.data?.transactionId || !event?.data?.userId) {
      ...
      const transaction: TransactionWithAccount | null = await db.transaction.findUnique({
        where: {
...
export const triggerRecurringTransactions = inngest.createFunction(
  ...
  async ({ step }: { step: any }) => {
    const recurringTransactions: Transaction[] = await step.run(
  ...
  async ({ step }: { step: any }) => {
    ...
    for (const budget of budgets) {
      await step.run(`check-budget-${budget.id}`, async () => {
        ...
        if (!defaultAccount) return; // Skip if no default account
        ...
        if (
          ...
          const budgetAlertData: BudgetAlertData = {
            percentageUsed,
